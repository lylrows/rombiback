...
namespace RombiBack.AWS.ROM.ENTEL_RETAIL.Services
{
    public class S3Service
    {
        ...
        public async Task<string> UploadFileToS3Async(IFormFile pdf)
        {
            ...
            try
            {
                using (var stream = new MemoryStream())
                {
                    ...
                    var key = $"BundlesRombiRETAIL/content/{pdf.FileName}";
                    ...
                    var objectUrl = $"https://{_bucketName}.s3.amazonaws.com/BundlesRombiRETAIL/content/{pdf.FileName}";
        ...
        public async Task<string> UploadFileToS3AsyncRW(IFormFile pdf)
        {
            if (pdf == null || pdf.Length == 0)
                throw new ArgumentException("No se ha proporcionado un archivo PDF.");
            ...
            try
            {
                using (var stream = new MemoryStream())
                {
                    ...
                    var key = $"BundlesPDF/content/{pdf.FileName}";
                    ...
                    var objectUrl = $"https://{_bucketName}.s3.amazonaws.com/BundlesPDF/content/{pdf.FileName}";
                    var nombre = pdf.FileName;
                    // Crear un objeto JSON para la respuesta
                    var jsonResponse = new { status = "OK", message = "Archivo PDF cargado correctamente en Amazon S3.", nombrepdf = pdf.FileName, url = objectUrl, urlprefirmada = preSignedUrl, pdfBase64 = base64String };
                    return JsonConvert.SerializeObject(jsonResponse);
        ...
        public async Task<string> UploadImageToS3AsyncRetail(string base64Image, string voucherName)
        {
            ...
            try
            {
                ...
                {
                    var key = $"BoletasRombiRetail/content/{voucherName}";
        ...
        public string GetPreSignedUrlForVoucher(string voucherName, int expirationMinutes = 15)
        {
            string objectKey = $"BoletasRombiRetail/content/{voucherName}";
