...
{
    public class PermisosLimtekRepository : IPermisosLimtekRepository
    {
        private readonly DataAcces _dbConnection;

        public PermisosLimtekRepository(DataAcces dbConnection)
        {
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(_dbConnection.GetConnectionROMBI()))
                {
                    ...
                    using (SqlCommand command = new SqlCommand("USP_GETCODIGOS", connection))
                    {
                        ...
                        {
                            List<CodigosResponseLimtek> response = new List<CodigosResponseLimtek>();

                            while (await reader.ReadAsync())
                            {
                                CodigosResponseLimtek codigoResponse = new CodigosResponseLimtek
                                {
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(_dbConnection.GetConnectionROMBI()))
                {
                    ...
                    using (SqlCommand command = new SqlCommand("USP_GETALLUSERS", connection))
                    {
                        ...
                        using (SqlDataReader reader = await command.ExecuteReaderAsync())
                        {
                            List<AllUsersResponseLimtek> response = new List<AllUsersResponseLimtek>();

                            while (await reader.ReadAsync())
                            {
                                AllUsersResponseLimtek allusersResponse = new AllUsersResponseLimtek
                                {
                ...
                using (SqlConnection connection = new SqlConnection(_dbConnection.GetConnectionROMBI()))
                {
                    ...
                    using (SqlCommand command = new SqlCommand("USP_GETPERFILES", connection))
                    {
                        ...
                        using (SqlDataReader reader = await command.ExecuteReaderAsync())
                        {
                            if (reader.HasRows)
                            {
                                List<PerfilesLimtek> response = new List<PerfilesLimtek>();

                                while (await reader.ReadAsync())
                                {
                                    PerfilesLimtek perf = new PerfilesLimtek();
                            ...
                            else
                            {
                                ...
                                return new List<PerfilesLimtek>(); // Devuelve una lista vacía en lugar de null
                            }
                        }
                    }
                }
        ...
        {
            List<RespuestaLimtek> respuestas = new List<RespuestaLimtek>();
                    ...
                    foreach (var request in requests)
                    {
                        using (SqlCommand command = new SqlCommand("USP_VALIDARESTRUCTURAMODULOS", connection))
                        {
                            ...
                            using (SqlDataReader rdr = await command.ExecuteReaderAsync())
                            {
                                ...
                                {
                                    RespuestaLimtek respuesta = new RespuestaLimtek
                                    {
